                   ...
                   const CameraParam* cameraParam)
{
  ...
  // 画像メモリを確保して背景エッジをグレーで描く
  bool color = true;
  IplImage* cvColorImage = createDebugImage(parameters, color, edge, 80);
  ...
  for (i = 0; i < stereo.numOfconics; i++)
    {
      ...
      vertex = &conic->work.vertex;
      if (conic->featureL->type != ConicType_Hyperbola || vertex->valid != 1)
        {
          continue;
        }
      // 頂点を黄色で描画
      projectXYZ2LR(&iPos, vertex->position, (CameraParam*)cameraParam);
      ix = roundoff(iPos.col);
      iy = roundoff(iPos.row);
      pt = cvPoint(ix, iy);
      cvLine(cvColorImage, pt, pt, yellow);
      // 端点１を緑色で描画
      projectXYZ2LR(&iPos, vertex->endpoint1, (CameraParam*)cameraParam);
      ix = roundoff(iPos.col);
      iy = roundoff(iPos.row);
      pt = cvPoint(ix, iy);
      cvLine(cvColorImage, pt, pt, green);
      // 端点２を赤色で描画
      projectXYZ2LR(&iPos, vertex->endpoint2, (CameraParam*)cameraParam);
      ix = roundoff(iPos.col);
      iy = roundoff(iPos.row);
      pt = cvPoint(ix, iy);
      cvLine(cvColorImage, pt, pt, red);
  ...
  outDebugImage(cvColorImage, "vertices3D", pairing, parameters.dbgdisp);

  return 0;
}

int
drawStereoCircles(const uchar* edge,
                  const StereoData& stereo,
                  int pairing,
                  const Parameters& parameters,
                  const CameraParam* cameraParam)
{
  // ３次元円復元結果画像（左画像）の表示・保存
  // ３次元復元結果変数：stereo
  ...
  for (i = 0; i < stereo.numOfconics; i++)
    {
      conic = &stereo.conics[i];
      circle = &(stereo.conics[i].work.circle);
      if (conic->type != ConicType_Ellipse || circle->valid != 1)
        {
          continue;
        }
...
int
printStereoVertices(const StereoData& stereo, int pairing)
{
  // 頂点を構成する2ベクトルの保存
  ...
  snprintf(filename, sizeof(filename), "vertex3D%d.txt", pairing);
  fp = fopen(filename, "w");
  if (fp == NULL)
    {
  ...
  for (i = 0; i < stereo.numOfconics; i++)
    {
      ...
      fprintf(fp, "%f %f %f\n", vertex->endpoint1[0], vertex->endpoint1[1], vertex->endpoint1[2]);
      fprintf(fp, "%f %f %f\n", vertex->position[0], vertex->position[1], vertex->position[2]);
      fprintf(fp, "%f %f %f\n", vertex->endpoint2[0], vertex->endpoint2[1], vertex->endpoint2[2]);
    }
  fclose(fp);
  return 0;
}
...
int
printStereoCircles(const StereoData& stereo, int pairing)
{
  // 円の保存
  FILE* fp;
  int i;
