                   ...
                   StereoPairing& pairing)     // ステレオペアの情報
{
  ...
  numOfResults = ns * ns * nm;

  memory = (MatchResult*) malloc(numOfResults * sizeof(MatchResult));
  if (memory == NULL)
    {
      goto ending;
    }

  memset(memory, 0x0, numOfResults * sizeof(MatchResult));
  ...
  memory = (MatchResult*) realloc(memory, sizeof(MatchResult) * p);
  if (memory)
    {
      Match.Results = memory;
    }
  ...
  getResultScore(memory, p, &model, pairing, score_weight);
