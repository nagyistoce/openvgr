              ...
              double tolerance, StereoPairing pairing)
{
  ...
  int numOfVerResults = scene.numOfVertices * model.numOfVertices;
  int i, j, k, status;
	     ...
	     double tolerance, StereoPairing pairing)
{
  Match3Dresults Match = { 0 };
  MatchResult* memory = NULL;
  int numOfCirResults = scene.numOfCircles * model.numOfCircles;
  int i, j, k, status;
  double score_weight = 1.0;    // 2円を差別化するために導入
...
//      double pdist = parameters.match.pdist;
//      int search = parameters.match.search;
//      int edge   = parameters.match.edge;
  StereoPairing pairing = parameters.pairing;
//      CalibParam* calibParam = model.calib;
  ...
  if (Match.error == 0 && scene.numOfCircles > 0 && model.numOfCircles > 0)
    {
      ...
      if (mergeMatch3Dresults(&Match, &cir2Match))
        {
  ...
  if (Match.error == 0 && scene.numOfCircles > 0 && model.numOfCircles > 0)
    {
      ...
      if (mergeMatch3Dresults(&Match, &cir1Match))
        {
